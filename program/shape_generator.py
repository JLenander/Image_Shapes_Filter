...
class random_shape_selector(shape_selector):
    ...
    def __init__(self, bbox: list[tuple[int]] | list[int], min_size: int, max_size: int) -> None:
        ...
        if isinstance(bbox[0], tuple):
            try:
                ...
                logging.error(f'Invalid 2-tuple bounding box {bbox=} for {type(self)}')
                raise ValueError('Invalid 2-tuple bounding box format')
        else:
            try:
                x0, y0, x1, y1 = bbox
            except ValueError:
                logging.error(f'Invalid 4-int bounding box {bbox=} for {type(self)}')
                raise ValueError('Invalid 4-int bounding box tuple format')
        if x0 >= x1 or y0 >= y1:
            logging.error(f'Invalid order of bbox coordinates {bbox=} for {type(self)}')
            raise ValueError('Invalid bounding box, point x0 should be < x1 and y0 should be < y1')
        self.bbox = [x0, y0, x1, y1]

        if min_size >= max_size:
            logging.error(f'Invalid parameters {min_size=} >= {max_size=} for {type(self)}')
            raise ValueError('min_size is not less than max_size')
        if min_size <= 0:
            logging.error(f'Invalid min_size (too small) {min_size=} {max_size=} for {type(self)}')
            raise ValueError('min_size must be at least 1 pixel')
        if min_size >= (x1 - x0) or min_size >= (y1 - y0):
            logging.error(f'Invalid min_size (not in bbox) {min_size=} {bbox=} for {type(self)}')
    ...
    def random_shape(self, target: Image.Image) -> tuple:
        ...
        if fill is False:  # Ellipse is not within image boundaries
            logging.debug(f'Shape ellipse turned into rectangle with {xy=}')
    ...
    def evolve_shape(self, target: Image.Image, shape: tuple) -> tuple:
        ...
        if new_color is False:  # Ellipse is not within image boundaries
            logging.debug(f'Shape ellipse evolved into rectangle with {new_xy=}')
            shape_name = 'rectangle'
            new_color = self._get_average_color(target, shape_name, new_xy)
        ...
        match shape_name:
            case 'ellipse':
                ...
                if mask.getextrema() == (0, 0):
                    logging.info(f'Ellipse not within image with properties {xy=} {self.bbox=}')
                    return False
            ...
            case 'rectangle':
                ...
                return average_color(target, mask)
            case _:
                logging.error(f'Invalid parameter {shape_name=} for {type(self)}')
    ...
    def draw_shape(img: Image.Image, shape: tuple) -> None:
        ...
        match shape_name:
            ...
            case _:
                logging.error(f'Invalid shape_name in {shape=} for random_shape_selector')
    ...
    def shape_str_to_tuple(shape: str) -> tuple:
        ...
        try:
            ...
            logging.error(f'Failed to properly unpack shape string {shape=}'
                          ' for random_shape_selector')
            raise ValueError(f'Invalid shape string format {shape=}')
        shape_tuple = random_shape_selector.shape_tuple
